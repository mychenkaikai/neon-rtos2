...
fn test_schedule_loop() {
    ...
    // 验证调度器正常工作,初始状态下当前任务应该是idle任务
    let current_task = Scheduler::get_current_task();
    assert_eq!(current_task.get_taskid(), 0); // idle任务ID为0

    // 触发调度，切换到第一个优先级最高的任务（task1）
    Scheduler::task_switch();
    let current_task = Scheduler::get_current_task();
    assert_eq!(current_task.get_taskid(), task1.get_taskid());

    // 验证调度器正常工作,应该是第二个任务
    Scheduler::task_switch();
    let current_task = Scheduler::get_current_task();
    assert_eq!(current_task.get_taskid(), task2.get_taskid());

    // 验证调度器正常工作,应该是第三个任务
    Scheduler::task_switch();
    let current_task = Scheduler::get_current_task();
    assert_eq!(current_task.get_taskid(), task3.get_taskid());

    // 验证调度器正常工作,应该是第四个任务
    Scheduler::task_switch();
    let current_task = Scheduler::get_current_task();
    assert_eq!(current_task.get_taskid(), task4.get_taskid());

    // 验证调度器正常工作,应该是第五个任务
    Scheduler::task_switch();   
    let current_task = Scheduler::get_current_task();
    assert_eq!(current_task.get_taskid(), task5.get_taskid());

    // 验证调度器正常工作,应该是第六个任务
    Scheduler::task_switch();
    let current_task = Scheduler::get_current_task();
    assert_eq!(current_task.get_taskid(), task6.get_taskid());

    // 验证调度器正常工作,应该是第七个任务
    Scheduler::task_switch();
    let current_task = Scheduler::get_current_task();
    assert_eq!(current_task.get_taskid(), task7.get_taskid());

    // 验证调度器正常工作,应该是第八个任务
    Scheduler::task_switch();
    let current_task = Scheduler::get_current_task();
    assert_eq!(current_task.get_taskid(), task8.get_taskid());

    // 再切换一次，调度器应该回到idle任务（因为idle任务也处于就绪状态，ID最小）
    Scheduler::task_switch();
    let current_task = Scheduler::get_current_task();
    assert_eq!(current_task.get_taskid(), 0);
