//! # Signal V2 - 支持闭包传递的信号量
//!
//! 基于 Arc 的信号量实现，无需全局变量，可以通过闭包传递。
//!
//! ## 设计思路
//!
//! 传统 Signal 使用全局 ID 来标识，必须用全局变量或宏定义。
//! Signal V2 使用 `Arc<SignalInner>` 的内存地址作为唯一标识，
//! 可以像 `std::sync::mpsc::channel` 一样在局部创建并传递给任务。
//!
//! ## 使用示例
//!
//! ```rust,no_run
//! use neon_rtos2::sync::SignalV2;
...
impl SignalV2 {
    ...
    ///
    /// // 创建一个已有 3 个信号的信号量
    /// let signal = SignalV2::with_count(3);
...
// 实现 Debug trait 方便调试
impl core::fmt::Debug for SignalV2 {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("SignalV2")
            .field("id", &self.id())
...
/// # 示例
/// ```rust,no_run
/// use neon_rtos2::sync::signal_v2::signal_pair;
///
...
/// use neon_rtos2::sync::SignalV2;
///
/// async fn wait_for_signal(signal: &SignalV2) {
///     signal.wait_async().await.unwrap();
///     // 收到信号后继续执行
/// }
/// ```
pub struct SignalFutureV2<'a> {
    signal: &'a SignalV2,
    registered: bool,
}

impl<'a> SignalFutureV2<'a> {
    fn new(signal: &'a SignalV2) -> Self {
        Self {
            signal,
            registered: false,
        }
...
impl<'a> core::future::Future for SignalFutureV2<'a> {
    type Output = Result<()>;
...
/// 带超时的异步等待 Future
pub struct SignalTimeoutFutureV2<'a> {
    signal: &'a SignalV2,
    deadline: usize,
    registered: bool,
}

impl<'a> SignalTimeoutFutureV2<'a> {
    fn new(signal: &'a SignalV2, timeout_ms: usize) -> Self {
        Self {
            signal,
...
impl<'a> core::future::Future for SignalTimeoutFutureV2<'a> {
    type Output = Result<()>;
...
/// 拥有所有权的异步等待 Future
///
/// 与 `SignalFutureV2` 不同，`OwnedSignalFuture` 持有 `Arc<SignalInner>` 的所有权，
/// 因此可以被存储在结构体中或跨 await 点持有。
///
/// # 示例
/// ```rust,no_run
/// use neon_rtos2::sync::signal_v2::OwnedSignal;
///
/// async fn example() {
...
impl SignalV2 {
    ...
    /// use neon_rtos2::sync::SignalV2;
    ///
    /// async fn example(signal: SignalV2) {
    ///     signal.wait_async().await.unwrap();
    ///     // 收到信号
    /// }
    /// ```
    pub fn wait_async(&self) -> SignalFutureV2<'_> {
        SignalFutureV2::new(self)
    }
    ...
    /// - `Err(RtosError::Timeout)`: 等待超时
    /// - `Err(RtosError::SignalClosed)`: 信号量已关闭
    pub fn wait_async_timeout(&self, timeout_ms: usize) -> SignalTimeoutFutureV2<'_> {
        SignalTimeoutFutureV2::new(self, timeout_ms)
    }
}
...
/// 拥有所有权的信号量
/// 
/// 与 `SignalV2` 不同，`OwnedSignal` 可以被 move 到其他任务中，
/// 因为它持有 `Arc` 的所有权而不是引用。
/// 
/// # 示例
/// ```rust,no_run
...
impl OwnedSignal {
    ...
    /// 从 SignalV2 转换
    pub fn from_signal(signal: SignalV2) -> Self {
        Self { inner: signal.inner }
    }

    /// 转换为 SignalV2
    pub fn into_signal(self) -> SignalV2 {
        SignalV2 { inner: self.inner }
    }

    /// 获取 SignalV2 引用
    pub fn as_signal(&self) -> SignalV2 {
        SignalV2 { inner: Arc::clone(&self.inner) }
    }
...
/// ```
pub fn signal_with_count(count: usize) -> SignalV2 {
    SignalV2::with_count(count)
}
...
mod tests {
    ...
    fn test_signal_v2_debug() {
        ...
        assert!(debug_str.contains("SignalV2"));
